...
impl RtcTransportBuilder {
    ...
    /// server runtime in terms of media transport policy rather than RTC adapter
    /// internals.
    #[must_use]
...
impl NegotiationPort for RtcTransport {
    async fn create_initial_session_offer(
        ...
        self.shards.create_initial_session_offer(session_key).await
    }
...
impl SessionPort for RtcTransport {
    async fn close_session(
        &self,
        session_key: &TransportSessionKey,
    ) -> Result<(), TransportAdapterError> {
        self.shards.close_session(session_key).await
    }
}

impl MediaPort for RtcTransport {
    ...
    async fn set_consumer_packet_gates(
        ...
        self.shards.set_consumer_packet_gates(updates).await
    }
...
impl ObservabilityPort for RtcTransport {
    fn transport_bitrate_snapshot(
        &self,
        session_keys: &[TransportSessionKey],
    ) -> TransportBitrateSnapshot {
        self.shards.transport_bitrate_snapshot(session_keys)
    }
    ...
    async fn active_speaker_diagnostic_snapshot(&self) -> Vec<ActiveSpeakerSourceDiagnostic> {
        self.shards.active_speaker_diagnostic_snapshot().await
    }

    async fn next_active_speaker_deadline(&self) -> Option<Instant> {
        self.shards.next_active_speaker_deadline().await
    }
    ...
    fn session_transport_health(
        ...
        self.shards.session_transport_health(session_key)
    }
}

impl SourcePolicyPort for RtcTransport {
    fn source_policy_subscription(&self) -> SourcePolicyUpdateSubscription {
        self.shards.source_policy_subscription()
    }
}
