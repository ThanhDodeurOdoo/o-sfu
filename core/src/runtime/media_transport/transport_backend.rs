//! Production backend selection for [`MediaTransport`](super::MediaTransport).
//!
//! The opaque media transport handle delegates to this module after it has
//! added caller-facing warning logs. In a normal production build there is only
//! one backend, [`RtcTransport`]. The separate test cfg module adds
//! deterministic backends without putting test code in the production boundary
//! file.
...
/// Production transport backend hidden behind the opaque media handle.
///
/// Keeping this enum private lets the core retain one delegation path without
/// exposing backend variants to server orchestration or external callers.
#[derive(Debug, Clone)]
pub(super) enum MediaTransportBackend {
    /// Str0m-backed RTC transport used by the real server.
    Rtc(RtcTransport),
}
