...
use crate::{
    ...
    runtime::{
        media_transport::{SessionUploadSlot, TransportMediaId},
        rtc_engine::client_rtp_capabilities_from_answer,
    },
};
...
async fn rtc_initial_session_offer_advertises_h264_simulcast_when_vp8_is_disabled() {
    let adapter = rtc_engine_with_codec_flags(
...
async fn rtc_initial_session_offer_reports_configured_codec_preferences() {
    let adapter = rtc_engine_with_codec_policy(
        MediaCodecFlags::default()
            .with_h264(true)
            .with_vp9(true)
            .with_av1(true),
