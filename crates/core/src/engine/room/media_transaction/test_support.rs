...
impl PendingPublishTransaction {
    pub(in crate::engine::room) async fn commit_with_parameters(
        self,
...
#[cfg(test)]
impl Room {
    pub(in crate::engine::room) fn stage_duplicate_after_next_publish_reservation_for_test(
        &self,
    ...
    pub(in crate::engine::room) fn duplicate_staged_publish_cleanup_target_for_test(
        &self,
    ) -> Option<TransportMediaId> {
        *lock_unpoisoned(&self.duplicate_staged_publish_cleanup_target)
    }

    pub(in crate::engine::room) fn inject_duplicate_staged_publish_after_reservation_for_test(
        &self,
    ...
    pub(in crate::engine::room) async fn staged_publish_count_for_connection(
        &self,
        user_id: &UserId,
        connection_id: ConnectionId,
    ) -> usize {
    ...
    pub(in crate::engine::room) async fn staged_publish_transport_media_id(
        &self,
        user_id: &UserId,
        connection_id: ConnectionId,
        stream_type: TestSourceKind,
