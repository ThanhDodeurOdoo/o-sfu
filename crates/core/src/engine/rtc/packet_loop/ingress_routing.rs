...
#[derive(Clone, Copy)]
pub(in crate::engine::rtc) struct PacketRouteDatagram<'a> {
...
pub(in crate::engine::rtc) fn route_packet_to_matching_session_at(
    state: &mut PacketLoopState,
    snapshot_state: &Arc<Mutex<RtcSnapshotState>>,
    demux: &mut DemuxRecoveryState,
    metrics: &RtcMetricsRecorder,
...
mod tests {
    ...
    use crate::engine::rtc::test_support::serialize_stun_message;
