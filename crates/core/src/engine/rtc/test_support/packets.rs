...
#[cfg(test)]
#[must_use]
pub(in crate::engine::rtc) fn sample_rtp_packet(sequence_number: u16, ssrc: u32) -> Vec<u8> {
    sample_rtp_packet_with_len(sequence_number, ssrc, RTP_HEADER_LEN)
}

#[must_use]
pub(in crate::engine::rtc) fn sample_rtp_packet_with_len(
    sequence_number: u16,
    ssrc: u32,
    packet_len: usize,
) -> Vec<u8> {
...
pub(in crate::engine::rtc) fn serialize_stun_message(
