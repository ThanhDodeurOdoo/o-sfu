...
impl RequestTracker {
    ...
    #[cfg(feature = "verification-models")]
    pub(super) fn timeout_count(&self) -> usize {
        self.request_timeouts.len()
    }

    #[cfg(feature = "verification-models")]
    pub(super) fn contains_pending_request(&self, request_id: &RequestId) -> bool {
        self.pending_requests.contains_key(request_id)
    }

    #[cfg(feature = "verification-models")]
    pub(super) fn contains_timeout_timer(&self, timer_id: u32) -> bool {
        self.request_timeouts.contains_key(&timer_id)
    }

    #[cfg(feature = "verification-models")]
    pub(super) fn has_bijection_between_requests_and_timers(&self) -> bool {
        if self.pending_requests.len() != self.request_timeouts.len() {
            return false;
        }
        for (request_id, pending_request) in &self.pending_requests {
            let Some(mapped_request_id) =
                self.request_timeouts.get(&pending_request.timeout_timer_id)
            else {
                return false;
            };
        ...
        for (timer_id, request_id) in &self.request_timeouts {
            let Some(pending_request) = self.pending_requests.get(request_id) else {
                return false;
            };
            if pending_request.timeout_timer_id != *timer_id {
                return false;
            }
        }
        true
