...
fn derive_consumable_parameters_remap_primary_payloads_rtx_apt_and_bindings() {
    ...
    assert_eq!(
        formats.first().map(|format| format.payload_type()),
        Some(101)
    );
    assert_eq!(
        formats.get(1).map(|format| format.payload_type()),
        Some(102)
    );
    assert_eq!(
        formats.get(1).and_then(|format| {
            format
                .parameters()
...
fn consumer_negotiation_filters_rtx_bindings_when_apt_does_not_match() {
    let consumable_parameters = MediaStream::new(
        ...
        vec![],
        vec![
            StreamBinding::new().with_ssrc(5_678).with_payload_type(96),
            StreamBinding::new().with_ssrc(5_679).with_payload_type(97),
