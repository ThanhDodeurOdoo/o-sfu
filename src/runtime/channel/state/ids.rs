...
pub(in crate::runtime::channel) struct ProducerRuntimeId(u64);

impl ProducerRuntimeId {
    pub(in crate::runtime::channel) fn allocate(next_producer_id: &mut u64) -> Self {
        let current = *next_producer_id;
        *next_producer_id = next_producer_id.saturating_add(1);
        Self(current)
    }

    pub(in crate::runtime::channel) fn into_wire_id(self) -> String {
        self.to_string()
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub(in crate::runtime::channel) struct ConsumerRuntimeId(u64);

impl ConsumerRuntimeId {
    pub(in crate::runtime::channel) fn allocate(next_consumer_id: &mut u64) -> Self {
