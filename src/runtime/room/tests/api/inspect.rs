...
#[derive(Clone, Copy)]
pub(crate) struct RoomTestInspect<'a> {
    pub(super) room: &'a Room,
}

impl RoomTestInspect<'_> {
    pub(crate) async fn router_user_count(self) -> usize {
    ...
    pub(crate) async fn session_has_parsed_client_rtp_capabilities(self, user_id: &UserId) -> bool {
        self.room
            .state
            .read()
            .await
    ...
    pub(crate) async fn user_connection_id(self, user_id: &UserId) -> Option<ConnectionId> {
    ...
    pub(crate) async fn producer_owner_user_id_for_transport_media_id(
        self,
        transport_media_id: TransportMediaId,
    ) -> Option<UserId> {
        self.room
    ...
    pub(crate) async fn user_info_snapshot(self, user_id: &UserId) -> Option<(UserId, UserInfo)> {
        self.room.state.read().await.user_info_snapshot(user_id)
    }

    pub(crate) async fn has_session(self, user_id: &UserId) -> bool {
